...
fig2_b <- ggplot(d, aes(size, PIK3CA_amp)) + geom_point(aes(shape = factor(hist_group)), show.legend = T) +
  facet_wrap(~BenignCA) +
  labs(x = NULL , y="PIK3CA amplification", shape = NULL) + 
  theme_bw()

fig2_c <- ggplot(d, aes(size, MT_copynumber)) + geom_point(aes(shape = factor(hist_group)), show.legend = T) +
  facet_wrap(~BenignCA) +
  labs(x ="Size (cm)", y="Mitochondria copy number", shape = NULL) + 
  theme_bw() + scale_y_log10()

figure2 <- arrangeGrob(fig2_a, fig2_b, fig2c, nrow = 3)
...
ggsave("Fig4.tiff", units = "in", width = 5, height = 4, dpi =300)
```
